...
url = "https://drive.google.com/file/d/1G6vr0WoVW04ztBGRAZMr3lSRBrYOnIEP/view?usp=sharing"
file_id = url.split('/')[-2]
...
# min_date dan max_date harus dalam format datetime
...
with st.sidebar:
    ...
    st.image("https://raw.githubusercontent.com/UsamahPutraFirdaus/Submission_Analysis_Data_Python/main/Dashboard/logo_olist.png")
    ...
    st.write('Copyright (C) © 2023 by Usamah ')
...
for x, y in zip(daily_orders_df["order_approved_at"], daily_orders_df["order_count"]):
    label = f"{y}"
...
plt.plot(
    customer_spend_df['order_approved_at'],
    customer_spend_df['total_spend'],
    marker='o',
    linestyle='-',
    color="#90CAF9",
    label='Total Spend'
...
for x, y in zip(customer_spend_df['order_approved_at'], customer_spend_df['total_spend']):
    label = f"{y:.2f}"
...
with col1:
    ...
    st.markdown(f"Highest Number : **{highest_product_sold}**")
...
# Plot for highest sold products
sns.barplot(
    x="product_id",
...
# Add data labels to the highest sold products
for i, value in enumerate(most_and_least_products_df.head(5)['product_id']):
    ax[0].text(value, i, str(value), ha='center', va='center', fontsize=12, color='black')

# Plot for lowest sold products
sns.barplot(
    x="product_id",
    y="product_category_name_english",
...
# Add data labels to the lowest sold products
for i, value in enumerate(most_and_least_products_df.sort_values(by="product_id", ascending=True).head(5)['product_id']):
    ax[1].text(value, i, str(value), ha='center', va='center', fontsize=12, color='black')
